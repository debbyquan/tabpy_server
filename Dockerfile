...
COPY Caddyfile /etc/tabpy_server/Caddyfile
COPY start.sh /tabpy_server/start.sh
RUN chmod +x /tabpy_server/start.sh

# Optional: Railway doesn’t require EXPOSE, but if you keep it, expose the public listener.
EXPOSE 8080

CMD ["/tabpy_server/start.sh"]
